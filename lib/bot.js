...
class Bot {
  ...
  sendToIRC(message) {
    ...
    var displayName = user.profile.display_name;
    if (displayName == "") displayName = user.profile.real_name;
    if (displayName == "") displayName = user.name;

    const username = this.ircUsernameFormat.replace(/\$username/g, displayName);
    ...
    if (ircChannel) {
      ...
      if (this.isCommandMessage(text)) {
      ...
      } else if (message.subtype && message.subtype === 'me_message') {
        text = `${username}/me ${text}`;

        logger.debug('Sending message to IRC', channelName, text);
        this.ircClient.say(ircChannel, text);
      } else if (message.files && message.files.length > 0) {
        let private_url = message.files[0].permalink;
        let public_url = message.files[0].permalink_public;
        let filename = private_url.substring(private_url.lastIndexOf('/')+1);
        ...
        .then((result) => {
          text = `${username}[File ${filename}] ${public_url}${text}`;
          logger.debug('Sending message to IRC', channelName, text);
          this.ircClient.say(ircChannel, text);
        })
        .catch(err => {
          logger.info(`Disabling public URL ${private_url}: ${err}`);
          text = `${username}[Private File] ${private_url}${text}`;
          logger.debug('Sending message to IRC', channelName, text);
          this.ircClient.say(ircChannel, text);
        });
      } else {
